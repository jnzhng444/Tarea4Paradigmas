...
static void parse_state_line(const char* line) {
    ...
    // Parsear solo MI jugador (ahora con coordenadas x,y en píxeles)
        ...
        if (pEnd) {
            ...
            while (item) {
                ...
                if (idbuf[0] && strcmp(idbuf, g_local_id) == 0) {
                    ...
                    if (liana_ptr) {
                        p->onLiana = (parse_float_loose(liana_ptr + 8) > 0.5f);
                    }
...
static void draw_player(PlayerState* p) {
    ...
    Vector2 center = {p->pos.x, p->pos.y + 15}; // offset para centrar
    
    DrawCircleV(center, 15, color);
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (g_world.playerCount > 0 && !won) {
            if (level_check_win(&g_level, g_world.players[0].pos)) {
                won = true;
